...
<Info>
**Declaring auth-aware clients in `config.json`.** All five auth types — including `oauth`, `per_user_oauth`, and `per_user_headers` — can be declared in `config.json`. The three that need an admin verification step (the two OAuth variants, plus `per_user_headers`) land in **`pending_verification`** state at boot. The MCP Gateway UI surfaces an **Authorize** / **Verify** CTA on each pending row that runs the same verification flow the Web UI Create form uses. See each auth type's page for the `config.json` shape.
</Info>
...
| `vk`       | Anyone holding the URL                            | Iff `mcp_enable_temp_token_auth=true` | No if temp token present; otherwise yes   |
| `session`  | Anyone holding the URL                            | Iff `mcp_enable_temp_token_auth=true` | No if temp token present; otherwise yes   |
