...
Once authorized, the OAuth credentials live in the encrypted `oauth_configs` table and the `pending_oauth_config_json` stash is cleared from the row. Rotate credentials through the dashboard or API; see [Token management](#token-management).
...
- `mcp_external_server_url` — what Bifrost uses for any consent URL it surfaces
