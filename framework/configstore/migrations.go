...
func migrationAddBudgetModelConfigIDColumn(ctx context.Context, db *gorm.DB) error {
	m := migrator.New(db, migrator.DefaultOptions, []*migrator.Migration{{
		...
		Rollback: func(tx *gorm.DB) error {
			tx = tx.WithContext(ctx)
			mig := tx.Migrator()
			// The legacy budget_id is left intact, so dropping the column is a clean reverse.
			if mig.HasColumn(&tables.TableBudget{}, "model_config_id") {
				if err := mig.DropColumn(&tables.TableBudget{}, "model_config_id"); err != nil {
					return fmt.Errorf("failed to drop model_config_id column: %w", err)
				}
			}
			return nil
		},
