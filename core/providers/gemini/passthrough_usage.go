...
func ExtractGeminiPassthroughUsage(path string, reqBody, accBody []byte) *schemas.BifrostPassthroughUsage {
	...
	if strings.Contains(path, "/interactions") {
		return extractGeminiInteractionsUsage(accBody)
	}
	...
	case "generateContent", "streamGenerateContent":
		return extractGeminiGenerateContentUsage(accBody)

	case "embedContent", "batchEmbedContents":
		return extractGeminiEmbeddingUsage(accBody)

	case "predict":
		return extractGeminiPredictUsage(reqBody, accBody)
...
func extractGeminiGenerateContentUsage(accBody []byte) *schemas.BifrostPassthroughUsage {
...
func extractGeminiEmbeddingUsage(accBody []byte) *schemas.BifrostPassthroughUsage {
	// Embeddings are never streamed, so accBody is plain JSON.
	if len(accBody) == 0 {
		return nil
	}

	var resp geminiPassthroughResp
	if err := sonic.Unmarshal(accBody, &resp); err != nil || resp.UsageMetadata == nil {
		return nil
	}
...
func extractGeminiInteractionsUsage(accBody []byte) *schemas.BifrostPassthroughUsage {
	body := providerUtils.LastSSEOrBody(accBody)
	if body == nil {
		return nil
	}
...
func extractGeminiPredictUsage(reqBody, accBody []byte) *schemas.BifrostPassthroughUsage {
	if len(accBody) > 0 {
	...
	return extractGeminiImagenUsage(reqBody, accBody)
}
...
func extractGeminiImagenUsage(reqBody, accBody []byte) *schemas.BifrostPassthroughUsage {
