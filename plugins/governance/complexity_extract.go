...
// must be text-only. Mixed or non-text user content is skipped so we do not
...
// The "prompt" field is analyzable only when it is a single string; array
// prompts represent batched completions and should not be merged for routing.
